...
import { claimAgentLead, manageListing, submitSupport, submitVerification, updateAgentJob, updateAssignedLead } from "../lib/accountActions";
...
export function AccountSectionScreen({ section, account, onBack, onRequireAuth }: AccountSectionScreenProps) {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      {data?.rows.length ? <><Text style={styles.sectionTitle}>Recent activity</Text><View style={styles.list}>{data.rows.map((row) => <View key={row.id} style={styles.row}><View style={styles.flex}><Text style={styles.itemTitle}>{row.title}</Text><Text style={styles.itemCopy}>{row.detail}</Text></View><Text style={styles.rowStatus}>{row.status.replaceAll("_", " ")}</Text>{section === "listings" && !row.status.startsWith("agent managed") ? <Pressable disabled={actionLoading} onPress={() => void listingAction(row.id, row.status)}><Text style={styles.link}>{row.status === "active" ? "Pause" : row.status === "draft" ? "Publish" : "Relist"}</Text></Pressable> : null}{section === "agent" && (row.id.startsWith("available-") || row.id.startsWith("lead-") || (row.id.startsWith("job-") && !row.status.startsWith("completed") && !row.status.startsWith("cancelled"))) ? <Pressable disabled={actionLoading} onPress={() => void agentAction(row.id, row.status)}><Text style={styles.link}>{row.id.startsWith("available-") ? "Claim" : row.id.startsWith("lead-") ? (row.status === "contacted" ? "Close" : "Contacted") : "Advance"}</Text></Pressable> : null}</View>)}</View></> : null}
