...
import { createHaziAccount, sendPasswordRecovery, signInWithEmail, updatePassword } from "../lib/auth";
import { colors } from "../theme";

type AuthScreenProps = { onClose: () => void; onAuthenticated: () => void; recoveryMode?: boolean };

export function AuthScreen({ onClose, onAuthenticated, recoveryMode = false }: AuthScreenProps) {
  ...
  async function submit() {
    setError(null);
    setMessage(null);
    if (!password || (!recoveryMode && !email.trim())) return setError(recoveryMode ? "Enter your new password." : "Enter your email and password.");
    ...
    try {
      if (recoveryMode) {
        await updatePassword(password); onAuthenticated();
      } else if (mode === "signin") {
  ...
  async function recover() {
    ...
    try { await sendPasswordRecovery(email); setMessage("Recovery email sent. Open its secure link on this phone."); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "Could not send the recovery email."); }
  ...
  return (
    <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.trust}><Ionicons name="shield-checkmark" size={30} color={colors.success} /><Text style={styles.trustTitle}>{recoveryMode ? "Choose a new password" : "Your trusted Hazi account"}</Text><Text style={styles.trustCopy}>{recoveryMode ? "Secure your Hazi account with a new strong password." : "Sign in to bid, sell, verify, fund your wallet, and manage protected auctions."}</Text></View>
        {!recoveryMode ? <View style={styles.switcher}><Pressable onPress={() => { setMode("signin"); setError(null); }} style={[styles.switch, mode === "signin" && styles.switchActive]}><Text style={[styles.switchText, mode === "signin" && styles.switchTextActive]}>Sign in</Text></Pressable><Pressable onPress={() => { setMode("signup"); setError(null); }} style={[styles.switch, mode === "signup" && styles.switchActive]}><Text style={[styles.switchText, mode === "signup" && styles.switchTextActive]}>Create account</Text></Pressable></View> : null}
        ...
        {!recoveryMode ? <Field icon="mail-outline" placeholder="Email address" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" /> : null}
        ...
        {mode === "signin" && !recoveryMode ? <Pressable disabled={loading} onPress={() => void recover()}><Text style={styles.recover}>Forgot password?</Text></Pressable> : null}
        ...
        <Pressable accessibilityRole="button" disabled={loading} onPress={submit} style={[styles.primaryButton, loading && styles.disabled]}>{loading ? <ActivityIndicator color={colors.white} /> : <Text style={styles.primaryText}>{recoveryMode ? "Save new password" : mode === "signin" ? "Sign in securely" : "Create Hazi account"}</Text>}</Pressable>
...
const styles = StyleSheet.create({
  ...
  recover: { color: colors.success, textAlign: "right", fontSize: 12, fontWeight: "800", marginTop: 10 },
