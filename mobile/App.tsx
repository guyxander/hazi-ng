    ...
    <SafeAreaProvider>
      <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
        ...
        {authVisible ? <AuthScreen recoveryMode={recoveryMode} onClose={() => { setAuthVisible(false); setRecoveryMode(false); }} onAuthenticated={() => { void refreshAccount(); setAuthVisible(false); setRecoveryMode(false); }} /> : <View style={styles.app}>
          ...
          ) : tab === "sell" ? <SellScreen account={account} onRequireAuth={() => setAuthVisible(true)} /> : tab === "activity" ? <ActivityScreen account={account} onRequireAuth={() => setAuthVisible(true)} /> : <ProfileScreen account={account} onRequireAuth={() => setAuthVisible(true)} onOpenSection={setAccountSection} onProfileSaved={refreshAccount} onSignOut={async () => { await signOutAccount(); setAccount(null); }} />}
